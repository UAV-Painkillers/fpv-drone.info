...
import { FlightLogEvent } from "./flightlog_event";
import {
...
export const FlightLogParser = function (logData) {
  //Private constants:
  var FLIGHT_LOG_MAX_FRAME_LENGTH = 256,
  ...
  var completeSysConfig = Object.assign(
    {},
    defaultSysConfig,
    defaultSysConfigExtension
  );
    ...
    switch (fieldName) {
      ...
      case "Firmware type":
        switch (fieldValue) {
          case "Cleanflight":
            ...
            console.log("html: remove class isBaseF");
            console.log("html: add class isCF");
            console.log("html: remove class isBF");
            console.log("html: remove class isINAV");
            break;
          default:
            that.sysConfig.firmwareType = FIRMWARE_TYPE_BASEFLIGHT;
            console.log("html: add class isBaseF");
            console.log("html: remove class isCF");
            console.log("html: remove class isBF");
            console.log("html: remove class isINAV");
        }
        break;
      ...
      case "anti_gravity_gain":
        if (
          ...
          semverGTE(that.sysConfig.firmwareVersion, "3.1.0") &&
          semverLT(that.sysConfig.firmwareVersion, "3.1.7")
        ) {
      ...
      case "dterm_lpf_hz":
        if (
          (that.sysConfig.firmwareType == FIRMWARE_TYPE_BETAFLIGHT &&
            semverGTE(that.sysConfig.firmwareVersion, "3.0.1")) ||
          (that.sysConfig.firmwareType == FIRMWARE_TYPE_CLEANFLIGHT &&
            semverGTE(that.sysConfig.firmwareVersion, "2.0.0"))
        ) {
          that.sysConfig[fieldName] = parseInt(fieldValue, 10);
        } else {
      ...
      case "Firmware revision":
        ...
        if (matches != null) {
          ...
          if (matches[1] === "Betaflight") {
            ...
            console.log("html: remove class isBaseF");
            console.log("html: remove class isCF");
            console.log("html: add class isBF");
            console.log("html: remove class isINAV");
          }
          ...
          that.sysConfig.firmwareVersion =
            that.sysConfig.firmware + "." + that.sysConfig.firmwarePatch;
        } else {
          /*
           * Try to detect INAV
           */
          ...
          if (matches != null) {
            ...
            console.log("html: remove class isCF");
            console.log("html: remove class isBF");
            console.log("html: add class isINAV");
          } else {
  ...
  function applyTimeRollover() {
    if (lastMainFrameTime != -1) {
      if (
        ...
        ) < castTo32BitUnsigned(lastMainFrameTime) &&
  ...
  function parseFrame(
    ...
    var predictor = frameDef.predictor,
  ...
  function parseEventFrame() {
    ...
    switch (eventType) {
      ...
      case FlightLogEvent.LOGGING_RESUME:
        ...
        lastEvent.data.currentTime =
          stream.readUnsignedVB() + timeRolloverAccumulator;
        break;
  ...
  this.parseLogData = function (raw, startOffset, endOffset) {
    var looksLikeFrameCompleted = false,
      prematureEof = false,
      frameStart = 0,
      frameType = null,
      lastFrameType = null;
