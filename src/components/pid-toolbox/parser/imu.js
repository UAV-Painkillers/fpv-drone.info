...
export function IMU(copyFrom) {
  ...
  function calculateHeading(vec, roll, pitch) {
    var cosineRoll = Math.cos(roll),
      ...
      heading =
        Math.atan2(headingY, headingX) + (magneticDeclination / 10.0) * RAD;
   ...
   */
  this.updateEstimatedAttitude = function (
    gyroADC,
    accSmooth,
    currentTime,
    acc_1G,
    ...
    if (72 < accMag && accMag < 133) {
      this.estimateGyro.X =
        (this.estimateGyro.X * gyro_cmpf_factor + accSmooth[0]) *
        INV_GYR_CMPF_FACTOR;
      this.estimateGyro.Y =
        (this.estimateGyro.Y * gyro_cmpf_factor + accSmooth[1]) *
        INV_GYR_CMPF_FACTOR;
      this.estimateGyro.Z =
        (this.estimateGyro.Z * gyro_cmpf_factor + accSmooth[2]) *
        INV_GYR_CMPF_FACTOR;
    }
