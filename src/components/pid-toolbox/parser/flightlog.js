...
import {
  ...
  FIRMWARE_TYPE_INAV,
} from "./firmware_types.js";
import { gte as semverGTE } from "semver";
import { FlightLogEvent } from "./flightlog_event.js";
import {
...
import { SUPER_EXPO_YAW } from "./superexpo_yaw.js";
...
export function FlightLog(logData) {
  var ADDITIONAL_COMPUTED_FIELD_COUNT = 15 /** attitude + PID_SUM + PID_ERROR + RCCOMMAND_SCALED + GYROADC_SCALED **/,
    that = this,
    logIndex = false,
  ...
  this.getLogError = function (logIndex) {
    var error = logIndexes.getIntraframeDirectory(logIndex).error;
  ...
  this.getStats = function (logIndex) {
    var rawStats = getRawStats(logIndex);
  ...
  this.getFrameAtTime = function (startTime) {
    var chunks = this.getChunksInTimeRange(startTime, startTime),
      chunk = chunks[0];
  ...
  this.getSmoothedFrameAtTime = function (startTime) {
    var chunks = this.getSmoothedChunksInTimeRange(startTime, startTime),
      chunk = chunks[0];
  ...
  function estimateNumCells() {
    ...
    if (
      (sysConfig.firmwareType == FIRMWARE_TYPE_BETAFLIGHT &&
        semverGTE(sysConfig.firmwareVersion, "3.1.0")) ||
      (sysConfig.firmwareType == FIRMWARE_TYPE_CLEANFLIGHT &&
        semverGTE(sysConfig.firmwareVersion, "2.0.0"))
    ) {
  ...
  function getChunksInIndexRange(startIndex, endIndex) {
    ...
    for (var chunkIndex = startIndex; chunkIndex <= endIndex; chunkIndex++) {
      ...
      if (chunk) {
          ...
          if (frameValid) {
            switch (frameType) {
              ...
              case "I":
                ...
                for (let i = 0; i < slowFrameLength; i++) {
        ...
        parser.setTimeRolloverAccumulator(
          iframeDirectory.initialTimeRolloverAccumulator[chunkIndex]
        );
  ...
  this.getChunksInTimeRange = function (startTime, endTime) {
    var startIndex = binarySearchOrPrevious(iframeDirectory.times, startTime),
      ...
      if (!destChunk.hasAdditionalFields) {
        ...
        for (var i = 0; i < sourceChunk.frames.length; i++) {
          ...
          attitude = chunkIMU.updateEstimatedAttitude(
            ...
            sysConfig.gyroScale,
            magADC
              ? [srcFrame[magADC[0]], srcFrame[magADC[1]], srcFrame[magADC[2]]]
              : false
          );
  ...
  function addMissingEventTimes(chunks, processLastChunk) {
     ...
     */
    var endChunk = processLastChunk ? chunks.length : chunks.length - 1;
  ...
  this.getSmoothedChunksInTimeRange = function (startTime, endTime) {
    ...
    for (
      var i = leadingROChunks;
      i < sourceChunks.length - trailingROChunks;
      i++
    ) {
      var sourceChunk = sourceChunks[i],
        ...
        mainLoop: for (
          ...
          for (
            ...
            while (
              ...
              var oldleftChunkIndex = leftChunkIndex,
  ...
  this.openLog = function (index) {
    const logError = this.getLogError(index);
    if (logError) {
    ...
    switch (this.getSysConfig().firmwareType) {
      case FIRMWARE_TYPE_CLEANFLIGHT:
        ...
        if (semverGTE(this.getSysConfig().firmwareVersion, "2.0.0")) {
          console.log("showing header dialog");
        } else {
          console.log("hiding header dialog");
        }
        break;
  ...
  if (
    ...
    var calculateSetpointRate = function (axis, rc) {
      ...
      if (rcExpo) {
        ...
        rcCommandf =
          rcCommandf * Math.pow(rcInput[axis], RC_EXPO_POWER) * expof +
          rcCommandf * (1 - expof);
      }
      ...
      if (sysConfig.rates[axis]) {
        rcSuperfactor =
          1.0 /
          constrain(
            1.0 - Math.abs(rcCommandf) * (sysConfig.rates[axis] / 100.0),
            0.01,
            1.0
          );
      ...
      else return constrain(angleRate, -1998.0, 1998.0); // Rate limit protection (deg/sec)
    };

    return calculateSetpointRate(axis, value);
  } else if (
    sysConfig.firmwareType == FIRMWARE_TYPE_BETAFLIGHT &&
    sysConfig.firmware >= 2.8
  ) {
    var that = this;
    ...
    var calculateRate = function (value, axis) {
      ...
      if (isSuperExpoActive()) {
        var rcFactor =
          axis === AXIS.YAW
            ? Math.abs(value) /
              (500.0 * (validate(sysConfig.rcYawRate, 100) / 100.0))
            : Math.abs(value) /
              (500.0 * (validate(sysConfig.rcRate, 100) / 100.0));
        rcFactor =
          1.0 /
          constrain(
